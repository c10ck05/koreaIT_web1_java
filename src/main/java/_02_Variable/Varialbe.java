package _02_Variable;

public class Varialbe {
    public static void main(String[] args) {
        // 변수 쓰는 이유
        System.out.println("hello");
        System.out.println("hello");
        System.out.println("hello");
        System.out.println("hello");
        System.out.println("hello");
        // hello -> world
        // 노가다 -> 변수를 통해 특정 값을 저장(Ram - 주기억장치)
        // ram에 데이터를 저장하는 행위

        // 자료형 변수명
        String greeting; // 변수 선언(빈 박스를 만들었다)

        // 변수 초기화
        // '='은 같다는 의미가 아니다
        // 우변의 데이터를 좌변의 변수에 저장하겠다.
        greeting = "hello";

        String academy = "Korea IT";

        // 변수 재활당(이전에 있던 hello는 사라짐)
        greeting = "World!";

        // 여러개를 한번에 선언 및 초기화
        String city = "부산", name = "이현재";

        /*
        명명법
        1. 필수 규칙
            String 1name -> 숫자로 시작할 수 없다.
            String my name -> 공백을 포함할 수 없다.
            String class -> 예약어, 키워드 사용 불가

        2. 관례(국룰)
            파스칼표기법 LastName, PascalCase - 클래스명, 파일명
            카멜표기법 lastName, pascalCase - 변수명, 함수(메소드)명
        3. 매너
            의미없는 변수명을 사용하지 말자
            ex) num1, a, b, c, temp (x)
            good) 현재 쇼핑카트에 담긴 물품 갯수
            nuberOfItemsCurrentlyInShoppingCart
         */
    }
}
